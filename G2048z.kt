...
fun waitForValidInput(): Int {
    val input = waitForInput()
    return if (isValidInput(input)) input else waitForValidInput()
}
 
fun isValidInput(input: Int): Boolean = arrayOf('h'.toInt(),'l'.toInt(),'j'.toInt(),'k'.toInt()).contains(input)
...
fun manipulateGrid(grid: Array<Array<Int>>, input: Int): Array<Array<Int>> = when (input) {
    'h'.toInt() -> shiftCellsLeft(grid)
    'j'.toInt() -> shiftCellsDown(grid)
    'l'.toInt() -> shiftCellsRight(grid)
    'k'.toInt() -> shiftCellsUp(grid)
    else -> throw IllegalArgumentException("Expected one of [h, l, j, k]")
}
